...
class ShowAttendandTell():
    ...
    def test(self, img, caption, test=False):
        ...
        total_score, sc_bl, sc_rg, sc_me, sc_cid, sc_spi = self.calculate_metric_scores(reference_dict, generated_dict)
        print("BLEU4: %.3f" % sc_bl[3])
        print("ROUGE: %.3f" % sc_rg)
        print("METEOR: %.3f" % sc_me)
        print("CiDEr: %.3f" % sc_cid)
        print("SPICE: %.3f" % sc_spi)
        self.me = sc_me
        if test:
            ...
            self.score['SPICE'] = sc_spi
            with open('./result/baseline_1h.txt', 'a') as f:
                f.write("BLEU4: %.3f\n" % sc_bl[3])
                f.write("ROUGE: %.3f\n" % sc_rg)
                f.write("METEOR: %.3f\n" % sc_me)
                f.write("CiDEr: %.3f\n" % sc_cid)
    ...
    def calculate_metric_scores(self, references, candidates):
        ...
        ####SPICE###
        pycoco_spice = Spice()
        sc_spi, _ = pycoco_spice.compute_score(
            references, candidates)
        spice_end = time.time()
        ...
        return total_score, sc_bl, sc_rg, sc_me, sc_cid, sc_spi
